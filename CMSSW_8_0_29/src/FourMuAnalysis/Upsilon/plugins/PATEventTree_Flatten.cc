...
void PATEventTree::beginJob() {
  ...
  fTree->Branch("HLTP_IsoMu24_PS", &fHLTP_IsoMu24_PS, "HLTP_IsoMu24_PS/i");
  ...
  fTree->Branch("HLTP_Dimuon0_Jpsi_Muon_Filters", fHLTP_Dimuon0_Jpsi_Muon_Filters, "fHLTP_Dimuon0_Jpsi_Muon_Filters[8]/O");
  fTree->Branch("HLTP_Dimuon10_Jpsi_Barrel_Filters", fHLTP_Dimuon10_Jpsi_Barrel_Filters, "HLTP_Dimuon10_Jpsi_Barrel_Filters[7]/O");
  //
  fTree->Branch("HLTP_IsoMu27_Filters", fHLTP_IsoMu27_Filters, "HLTP_IsoMu27_Filters[6]/O");
  fTree->Branch("HLTP_IsoMu24_Filters", fHLTP_IsoMu24_Filters, "HLTP_IsoMu24_Filters[6]/O");
...
*/
  ...
  if ( fHLT_Skim && !( fHLTP_Dimuon0_Upsilon_Muon[1] || fHLTP_Dimuon0_Jpsi_Muon[1] || fHLTP_IsoMu27[1]|| fHLTP_IsoMu24[1] )) return; 
...
void PATEventTree::init() {
  ...
  fHLTP_DoubleMu3_PS = fHLTP_DoubleMu6_PS = fHLTP_DoubleMu7_PS = fHLTP_Dimuon0_Upsilon_Muon_PS = fHLTP_Dimuon0_Jpsi_Muon_PS = fHLTP_Dimuon0_Jpsi_PS = fHLTP_Dimuon10_Jpsi_Barrel_PS = fHLTP_TripleMu5_PS = fHLTP_QuadMuon0_Dimuon0_Jpsi_PS = fHLTP_QuadMuon0_Dimuon0_Upsilon_PS = fHLTP_Trimuon2_Upsilon5_Muon_PS = fHLTP_TrimuonOpen_5_3p5_2_Upsilon_Muon_PS =fHLTP_IsoMu27_PS=fHLTP_IsoMu24_PS = fHLTP_Mu50_PS= 0;

  for (int i = 0; i < 3; i++) {
    fHLTP_DoubleMu3[i] = fHLTP_DoubleMu6[i] = fHLTP_DoubleMu7[i] = fHLTP_Dimuon0_Upsilon_Muon[i] = fHLTP_Dimuon0_Jpsi_Muon[i] = fHLTP_Dimuon0_Jpsi[i] = fHLTP_Dimuon10_Jpsi_Barrel[i] = fHLTP_TripleMu5[i] = fHLTP_QuadMuon0_Dimuon0_Jpsi[i] = fHLTP_QuadMuon0_Dimuon0_Upsilon[i] = fHLTP_Trimuon2_Upsilon5_Muon[i] = fHLTP_TrimuonOpen_5_3p5_2_Upsilon_Muon[i] = fHLTP_IsoMu27[i]=fHLTP_IsoMu24[i] = fHLTP_Mu50[i] = false;
  }
  for (int i=0; i<8; i++){
    fHLTP_Dimuon0_Upsilon_Muon_Filters[i]=false;
  }
  for (int i = 0; i < 5; i++) {
    fHLTP_DoubleMu3_Filters[i] = fHLTP_DoubleMu6_Filters[i] = fHLTP_DoubleMu7_Filters[i] = fHLTP_TripleMu5_Filters[i] = fHLTP_Dimuon0_Jpsi_Filters[i] = fHLTP_Dimuon0_Jpsi_Muon_Filters[i] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[i] = fHLTP_IsoMu27_Filters[i]=fHLTP_IsoMu24_Filters[i] = fHLTP_Mu50_Filters[i] = false;
  }
  fHLTP_IsoMu27_Filters[5]=fHLTP_IsoMu24_Filters[5]=fHLTP_Mu50_Filters[5]=fHLTP_Dimuon0_Jpsi_Filters[5] = fHLTP_Dimuon0_Jpsi_Filters[6] = fHLTP_Dimuon0_Jpsi_Muon_Filters[5] = fHLTP_Dimuon0_Jpsi_Muon_Filters[6] = fHLTP_Dimuon0_Jpsi_Muon_Filters[7] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[5] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[6] = false;
...
void PATEventTree::fillHLTPath(const unsigned int psSet, const edm::Handle<edm::TriggerResults>& hHLTresults, const trigger::TriggerEvent& triggerEvent) {
  ...
  for (unsigned int itrig = 0; itrig < validTriggerNames.size(); itrig++) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_Dimuon0_Jpsi_Muon")!=string::npos) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_Dimuon0_Upsilon_Muon")!=string::npos) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_IsoMu27")!=string::npos) {
      fHLTP_IsoMu27[0] = wasrun;
      fHLTP_IsoMu27[1] = accept;
      fHLTP_IsoMu27[2] = error;
      fHLTP_IsoMu27_PS = prescale;
    } else if (validTriggerNames[itrig].find("HLT_IsoMu24")!=string::npos) {
      fHLTP_IsoMu24[0] = wasrun;
      fHLTP_IsoMu24[1] = accept;
      fHLTP_IsoMu24[2] = error;
      fHLTP_IsoMu24_PS = prescale;
      
    }  else if (validTriggerNames[itrig].find("HLT_Mu50")!=string::npos) {
      fHLTP_Mu50[0] = wasrun;
      fHLTP_Mu50[1] = accept;
      fHLTP_Mu50[2] = error;
  ...
  for(uint filterIndex=0;filterIndex<triggerEvent.sizeFilters();++filterIndex){  //loop over all trigger filters in event (i.e. filters passed)
    ...
    } else if(label.find( "hltL1sTripleMu0orTripleMu500" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[0]=true;
    } else if(label.find( "hltPreDimuon0UpsilonMuon" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[1]=true;
    } else if(label.find( "hltTripleMuonL1Filtered0" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[2]=true;
    } else if(label.find( "hltTripleMuonL2PreFiltered0" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[3]=true;
    } else if(label.find( "hltTripleMuL3PreFiltered0" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[4]=true;
    } else if(label.find( "hltUpsilonMuonL3Filtered" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[5]=true;
    } else if(label.find( "hltDisplacedmumuVtxProducerUpsilonMuon" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[6]=true;
    } else if(label.find( "hltVertexmumuFilterUpsilonMuon" ) != string::npos ) {
      fHLTP_Dimuon0_Upsilon_Muon_Filters[7]=true;
      //end of Upsilon filter
      //addition of filter for Z trigger
    } else if(label.find( "hltL1sV0SingleMu22IorSingleMu25" ) != string::npos ) {
    ...
    } else if(label.find( "hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p09" ) != string::npos ) {
      fHLTP_IsoMu27_Filters[5]=true;
      //      
    } else if(label.find( "hltL1sSingleMu22" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[0]=true;
    } else if(label.find( "hltPreIsoMu24" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[1]=true;
    } else if(label.find( "hltL1fL1sMu22L1Filtered0" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[2]=true;
    } else if(label.find( "hltL2fL1sMu22L1f0L2Filtered10Q" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[3]=true;
    } else if(label.find( "hltL3fL1sMu22L1f0L2f10QL3Filtered24Q" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[4]=true;
    } else if(label.find( "hltL3crIsoL1sMu22L1f0L2f10QL3f24QL3trkIsoFiltered0p09" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[5]=true;
      //
...
void PATEventTree::fillHLT(const pat::TriggerObjectStandAloneCollection& HLTObjects) {
  ...
  for ( int ih = 0; ih < (int) HLTObjects.size(); ih++) {
    ...
    std::string filter3 = "hltVertexmumuFilterJpsiMuon";// The last trigger selected..
    std::string filter2 = "hltTripleMuL3PreFiltered0"; //General Trigger path which first trigger our Trigger  
    std::string filterZ = "hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p09"; //L3 Trigger Path for IsoMu27 trigger 
    std::string filterZ1 = "hltL3crIsoL1sMu22L1f0L2f10QL3f24QL3trkIsoFiltered0p09"; //L3 Trigger Path for IsoMu24  trigger 
    std::string filter3Ups = "hltVertexmumuFilterUpsilonMuon";// The last trigger selected.. 
    std::string filter2Ups = "hltTripleMuL3PreFiltered0"; //General Trigger path which first trigger our Trigger                
    //std::string path = "HLT_Dimuon0_Jpsi3p5_Muon2_v4";
  ...
  //cout<<"Number of matched muon in L3 and Vertexing in Z are -->"<<passZ<<endl; 
}
...
void PATEventTree::fillJPsiMuMuCand(const reco::CompositeCandidateCollection& JPsiCands, const reco::MuonRefVector& goodMuons, std::vector<TransientTrack>& t_tks) {
  ...
  for(unsigned int ijp = 0; ijp < JPsiCands.size(); ijp++) {
    ...
    if( fJPsiMuCategory[fJPsiN][0]>0 && fJPsiMuCategory[fJPsiN][1]>0 && fJPsiMass[0]>2.8 && fJPsiMass[0]<110. ) {//changed for Upsilon
